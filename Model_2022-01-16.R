...
## ======================================================================================
rm(list=ls())
## Load data table packages to allow for 'data.table' to be used.
library(data.table)
library(dplyr)
library(tidyverse)
library(ggplot2)
...
#write.csv(population_CA_2000_2010, "population_CA_2000_2010.csv")
monthly_pm2.5_CA_1999_2021$Year = substr(monthly_pm2.5_CA_1999_2021$Year_Month, 1, 4)
#CA_Data$Year = substr(CA_Data$Year_Month, 1, 4)
...
get.mav <- function(Dth_E, n=60) {
  require(zoo)
  if(is.na(Dth_E[1])) Dth_E[1:(n-1)] <- mean(Dth_E, na.rm = TRUE)
  Dth_E <- na.locf(Dth_E, na.rm = FALSE)
  if(length(Dth_E) < n) return(Dth_E)
  c(Dth_E[1:(n-1)], rollapply(Dth_E, width = n, mean, align = "right"))
}
...
CA_Data$expected_death_36mth <- unlist(aggregate(Deaths~ID, CA_Data, get.mav, na.action = NULL, n=36)$Deaths)
...
# Figure 3: Regions used in the ACI (Actuaries Climate Index Development and Design, 2019)

library(knitr)
library(ggplot2)
include_graphics("ACI_Region_Map.png")
...
plot_data2 <- melt(CA_Data[, c(4, 28, 30)], id="Year")
plot_data2 %>%
  group_by(Year, variable) %>%
  summarise(Average_Mortality = mean(value, na.rm=TRUE)) %>%
  ggplot(aes(x=Year, y=Average_Mortality, col=variable, group=variable)) +
  geom_line()+
  theme(legend.position = "bottom", aspect.ratio=15/35, axis.text.x = element_text(angle = 90, hjust = 1, size = 9, vjust = 0.5, margin = margin(2.5, 5, 5, 5))) +
  labs(y="Average Annual Death Rate per mille", x="Year", colour = "Legend") +
...
mod_lm5 <- lm(Excess_Death_Rate ~ max_pm2.5_t_2 + Excess_Death_Rate_Lagged + T10 + T90 + as.numeric(as.factor(Five.Year.Age.Groups.Code)) + GENDER + COUNTY + Year_Month, data = CA_Data)

mod_lm3_resid <- resid(mod_lm3)
...
MSE_lm4 <- RSS_lm3 / length(mod_lm4$residuals)
...
mod_gam3 <- gam(Excess_Death_Rate ~ max_pm2.5 + s(Excess_Death_Rate_Lagged) + s(T10) + s(T90) + 
                    s(as.numeric(as.factor(Five.Year.Age.Groups.Code))) + GENDER + s(COUNTY) + s(as.numeric(Year_Month)) , data = CA_Data)
mod_gam4 <- gam(Excess_Death_Rate ~ max_pm2.5_t_1 + s(Excess_Death_Rate_Lagged) + s(T10) + s(T90) + 
                  s(as.numeric(as.factor(Five.Year.Age.Groups.Code))) + GENDER + s(COUNTY) + s(as.numeric(Year_Month)) , data = CA_Data)
...
library(stargazer)
test <- stargazer(
  mod_lm3, mod_lm4, mod_lm5, type = "html",
  title = "Excess Mortality (t) fit using linear model and prior period deaths",
  column.labels = c("Linear Model (no lag)", "Linear Model (1 month lag)", "Linear Model (2 month lag)"),
  dep.var.labels=c("Excess Mortality (t)"),
  covariate.labels=c("Maximum PM2.5 Concentration (t)", "Maximum PM2.5 Concentration (t-1)", "Maximum PM2.5 Concentration (t-2)", "Excess Death Rate (t-1)", "Low Temperature Index (t)", "High Temperature Index (t)", "Age Band (t)","Gender","County","Month and Year (t)"),
...
plot(density(mod_lm3_resid))
qqnorm(mod_lm3_resid)

#MODEL COMPARE: LM AND GAM
AIC(mod_lm3)
summary(mod_lm3)$sp.criterion
summary(mod_lm3)$r.sq

AIC(mod_gam3)
summary(mod_gam3)$sp.criterion
summary(mod_gam3)$r.sq
...
grid_tune <- expand.grid(
  ...
  gamma = 0, #pruning should be tuned, c(0, 0.05, 0.1, 0.5, 0.7, 0.9, 1.0)
  colsample_bytree = 1, #c(0.4, 0.6, 0.8, 1.0) subsample ratio of columns for tree
  min_child_weight = 1, #c(1,2,3) the larger the more conservative the model; can be used as a stop
  subsample = 1 #c(0.5, 0.75, 1.0) used to prevent overfitting by sampling x%
)
...
gbmImp <- varImp(xgb_tune, scale = FALSE)
gbmImp
plot(gbmImp, top = 10)
...
change_pt_data_sandiego <- change_pt_data %>%
  filter(COUNTY_NAME == "San Diego")

write.csv(change_pt_data_sandiego, "change_pt_data_sandiego.csv")

library(ecp)
e.divisive(change_pt_data, sig.lvl = 0.05, R = 9, k = NULL, min.size = 2 , alpha = 1)
e.divisive(change_pt_data_sandiego, sig.lvl = 0.05, R = 9, k = NULL, min.size = 12 , alpha = 1)

library(changepoint)
#value.ts = ts(change_pt_data$Excess_Death_Rate, frequency=12, start=c(1999,1), end=c(2019,12)) #create subset by date and make it a time series
value.ts = ts(change_pt_data_sandiego$Excess_Death_Rate, frequency=12, start=c(1999,1), end=c(2019,12)) #create subset by date and make it a time series
plot(value.ts)
...
library(mcp)
change_pt_data_x <- change_pt_data
change_pt_data_x$Year_Month_Num <- as.numeric(change_pt_data_x$Year_Month)
change_pt_data_x <- change_pt_data_x %>% filter(!is.na(Year_Month_Num))

View(change_pt_data_x)

CA_Data_x <- CA_Data
CA_Data_x$Year_Month_Num <- as.numeric(CA_Data_x$Year_Month)
CA_Data_x <- CA_Data_x %>% filter(!is.na(Year_Month_Num))

#try this one - we want varying change points by county
# this model assumes we have a flat line and then a linear increase
model = list(
  Excess_Death_Rate ~ 1,
  1+(1|COUNTY_NAME) ~ 0 + Year_Month_Num
  #~ 1
)

# varying change points with multiple groupings
model = list(
  y ~ 1,  # int_1
...
# also try other models: e.g. 2 plateaus etc

empty = mcp(model, sample = FALSE)
ggplot(change_pt_data_x, aes(x=Year_Month, y=Excess_Death_Rate)) +
  geom_point() +
  facet_wrap(~COUNTY_NAME)

# ggplot(change_pt_data_sandiego, aes(x=Year_Month, y=Excess_Death_Rate)) +
#   geom_point() +
#   facet_wrap(~COUNTY_NAME)

fit = mcp(model, data = change_pt_data_x)
          #, par_x = "Year_Month_Num")
plot(fit)
plot(fit, facet_by = "COUNTY_NAME")
